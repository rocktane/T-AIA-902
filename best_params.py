...
def _checkpoint_filename(agent_name):
    slug = (
        agent_name.lower()
        .replace(" ", "_")
        .replace("-", "_")
...
def checkpoint_path_for(agent_name):
    return os.path.join(CHECKPOINTS_DIR, _checkpoint_filename(agent_name))


def resolve_checkpoint_path(entry):
    if not entry:
        return None
    ...
    if os.path.isabs(checkpoint):
        return checkpoint
    return os.path.join(base_dir, checkpoint)
...
def save_best_params(agent_name, params, metrics, agent=None):
    """
    ...
    if agent is not None:
        ...
        checkpoint_path = checkpoint_path_for(agent_name)
        agent.save_checkpoint(checkpoint_path)
    existing_last_benchmark = current.get("last_benchmark") if isinstance(current, dict) else None
    data[agent_name] = {
        "params": params,
        "metrics": metrics,
    ...
    if checkpoint_path is not None:
        data[agent_name]["checkpoint"] = os.path.relpath(
            checkpoint_path,
            os.path.dirname(os.path.abspath(__file__)),
        )
    elif isinstance(current, dict) and current.get("checkpoint"):
        data[agent_name]["checkpoint"] = current["checkpoint"]
